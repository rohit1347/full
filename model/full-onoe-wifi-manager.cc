...
#include "full-onoe-wifi-manager.h"
#include "ns3/simulator.h"
#include "ns3/log.h"
#include "ns3/uinteger.h"

NS_LOG_COMPONENT_DEFINE ("FullOnoeWifiRemoteStation");

namespace ns3 {

struct OnoeWifiRemoteStation : public FullWifiRemoteStation
{
...
NS_OBJECT_ENSURE_REGISTERED (FullOnoeWifiManager);

TypeId
FullOnoeWifiManager::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::FullOnoeWifiManager")
    .SetParent<FullWifiRemoteStationManager> ()
    .AddConstructor<FullOnoeWifiManager> ()
    .AddAttribute ("UpdatePeriod",
                   "The interval between decisions about rate control changes",
                   TimeValue (Seconds (1.0)),
                   MakeTimeAccessor (&FullOnoeWifiManager::m_updatePeriod),
                   MakeTimeChecker ())
    .AddAttribute ("RaiseThreshold", "Attempt to raise the rate if we hit that threshold",
                   UintegerValue (10),
                   MakeUintegerAccessor (&FullOnoeWifiManager::m_raiseThreshold),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("AddCreditThreshold", "Add credit threshold",
                   UintegerValue (10),
                   MakeUintegerAccessor (&FullOnoeWifiManager::m_addCreditThreshold),
                   MakeUintegerChecker<uint32_t> ())
  ;
  return tid;
}

FullOnoeWifiManager::FullOnoeWifiManager ()
{
}
FullWifiRemoteStation *
FullOnoeWifiManager::DoCreateStation (void) const
{
...
void
FullOnoeWifiManager::DoReportRxOk (FullWifiRemoteStation *station,
                               double rxSnr, FullWifiMode txMode)
{
}
void
FullOnoeWifiManager::DoReportRtsFailed (FullWifiRemoteStation *st)
{
  OnoeWifiRemoteStation *station = (OnoeWifiRemoteStation *)st;
  station->m_shortRetry++;
}
void
FullOnoeWifiManager::DoReportDataFailed (FullWifiRemoteStation *st)
{
  OnoeWifiRemoteStation *station = (OnoeWifiRemoteStation *)st;
  station->m_longRetry++;
}
void
FullOnoeWifiManager::DoReportRtsOk (FullWifiRemoteStation *station,
                                double ctsSnr, FullWifiMode ctsMode, double rtsSnr)
{
}
void
FullOnoeWifiManager::DoReportDataOk (FullWifiRemoteStation *st,
...
void
FullOnoeWifiManager::DoReportFinalRtsFailed (FullWifiRemoteStation *st)
{
  OnoeWifiRemoteStation *station = (OnoeWifiRemoteStation *)st;
  UpdateRetry (station);
  station->m_tx_err++;
...
void
FullOnoeWifiManager::DoReportFinalDataFailed (FullWifiRemoteStation *st)
{
  OnoeWifiRemoteStation *station = (OnoeWifiRemoteStation *)st;
  UpdateRetry (station);
  station->m_tx_err++;
...
void
FullOnoeWifiManager::UpdateRetry (OnoeWifiRemoteStation *station)
{
...
void
FullOnoeWifiManager::UpdateMode (OnoeWifiRemoteStation *station)
{
...
FullWifiMode
FullOnoeWifiManager::DoGetDataMode (FullWifiRemoteStation *st,
                                uint32_t size)
{
  OnoeWifiRemoteStation *station = (OnoeWifiRemoteStation *)st;
  UpdateMode (station);
...
FullWifiMode
FullOnoeWifiManager::DoGetRtsMode (FullWifiRemoteStation *st)
{
  OnoeWifiRemoteStation *station = (OnoeWifiRemoteStation *)st;
  UpdateMode (station);
...
bool
FullOnoeWifiManager::IsLowLatency (void) const
{
  return false;
}
