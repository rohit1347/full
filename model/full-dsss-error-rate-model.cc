...
#include "ns3/log.h"
#include "full-dsss-error-rate-model.h"
#include <cmath>

NS_LOG_COMPONENT_DEFINE ("FullDsssErrorRateModel");

namespace ns3 {

#ifndef FULL_ENABLE_GSL
const double FullDsssErrorRateModel::FULL_WLAN_SIR_PERFECT = 10.0;
const double FullDsssErrorRateModel::FULL_WLAN_SIR_IMPOSSIBLE = 0.1;
#endif

double
FullDsssErrorRateModel::DqpskFunction (double x)
{
...
double
FullDsssErrorRateModel::GetDsssDbpskSuccessRate (double sinr, uint32_t nbits)
{
...
double
FullDsssErrorRateModel::GetDsssDqpskSuccessRate (double sinr,uint32_t nbits)
{
...
double
FullDsssErrorRateModel::GetDsssDqpskCck5_5SuccessRate (double sinr,uint32_t nbits)
{
#ifdef FULL_ENABLE_GSL
  ...
  // The matlab model
  double ber;
  if (sinr > FULL_WLAN_SIR_PERFECT)
    {
      ber = 0.0;
    }
  else if (sinr < FULL_WLAN_SIR_IMPOSSIBLE)
    {
      ber = 0.5;
    }
  else
...
double
FullDsssErrorRateModel::GetDsssDqpskCck11SuccessRate (double sinr,uint32_t nbits)
{
#ifdef FULL_ENABLE_GSL
  // symbol error probability
...
double
FullDsssErrorRateModel::SymbolErrorProb16Cck (double e2)
{
  double sep;
  double error;
...
double FullDsssErrorRateModel::SymbolErrorProb256Cck (double e1)
{
