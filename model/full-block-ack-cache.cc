...
void
FullBlockAckCache::Init (uint16_t winStart, uint16_t winSize)
{
  m_winStart = winStart;
...
void
FullBlockAckCache::UpdateWithMpdu (const FullWifiMacHeader *hdr)
{
...
void
FullBlockAckCache::UpdateWithBlockAckReq (uint16_t startingSeq)
{
...
void
FullBlockAckCache::ResetPortionOfBitmap (uint16_t start, uint16_t end)
{
  uint32_t i = start;
...
bool
FullBlockAckCache::IsInWindow (uint16_t seq)
{
  return ((seq - m_winStart + 4096) % 4096) < m_winSize;
}

void
FullBlockAckCache::FillBlockAckBitmap (FullCtrlBAckResponseHeader *blockAckHeader)
{
