 ...
 */
#ifndef FULL_MAC_LOW_H
#define FULL_MAC_LOW_H
...
#include "full-block-ack-cache.h"
 ...
 */
class FullMacLowTransmissionListener
{
public:
  FullMacLowTransmissionListener ();
  virtual ~FullMacLowTransmissionListener ();
   ...
   * CtsTimeout.
   */
  virtual void GotCts (double snr, FullWifiMode txMode) = 0;
  /**
   ...
   * used.
   */
  virtual void GotAck (double snr, FullWifiMode txMode) = 0;
  /**
 ...
 * ns3::Dcf methods.
 */
class FullMacLowDcfListener
{
public:
  FullMacLowDcfListener ();
  virtual ~FullMacLowDcfListener ();
  /**
 ...
 */
class FullMacLowBlockAckEventListener
{
public:
  FullMacLowBlockAckEventListener ();
  virtual ~FullMacLowBlockAckEventListener ();
  /**
 ...
 */
class FullMacLowTransmissionParameters
{
public:
  FullMacLowTransmissionParameters ();
...
private:
  friend std::ostream &operator << (std::ostream &os, const FullMacLowTransmissionParameters &params);
  uint32_t m_nextSize;
  enum
  {
    ACK_NONE,
 ...
 */
class FullMacLow : public Object
{
public:
  typedef Callback<void, Ptr<Packet>, const FullWifiMacHeader*> MacLowRxCallback;

  FullMacLow ();
  virtual ~FullMacLow ();

  void SetPhy (Ptr<FullWifiPhy> phy);
  void SetWifiRemoteStationManager (Ptr<FullWifiRemoteStationManager> manager);
  ...
  Time GetDuplexEnd (void) const;
  void UpdateDuplexEnd (Time end);
  bool IsWaitingAckTimeout (void) const;
   ...
   */
  void SetRxCallback (Callback<void,Ptr<Packet>,const FullWifiMacHeader *> callback);
  /**
   * \param listener listen to NAV events for every incoming
   *        and outgoing packet.
   */
  void RegisterDcfListener (FullMacLowDcfListener *listener);
  ...
  Time CalculateTransmissionTime (Ptr<const Packet> packet,
                                  ...
                                  const FullMacLowTransmissionParameters& parameters) const;
  ...
  void StartTransmission (Ptr<const Packet> packet,
                          const FullWifiMacHeader* hdr,
                          FullMacLowTransmissionParameters parameters,
   ...
   */
  void ReceiveOk (Ptr<Packet> packet, double rxSnr, FullWifiMode txMode, FullWifiPreamble preamble);
  /**
   * \param packet packet received.
   * \param rxSnr snr of packet received.
   *
   ...
   */
  void CreateBlockAckAgreement (const FullMgtAddBaResponseHeader *respHdr,
   ...
   */
  void RegisterBlockAckListenerForAc (enum AcIndex ac, FullMacLowBlockAckEventListener *listener);
private:
  ...
  uint32_t GetSize (Ptr<const Packet> packet, const FullWifiMacHeader *hdr) const;
  Time NowUs (void) const;
  void ForwardDown (Ptr<const Packet> packet, const FullWifiMacHeader *hdr,
                    FullWifiMode txMode);
  Time CalculateOverallTxTime (Ptr<const Packet> packet,
                               const FullWifiMacHeader* hdr,
                               const FullMacLowTransmissionParameters &params) const;
  FullWifiMode GetRtsTxMode (Ptr<const Packet> packet, const FullWifiMacHeader *hdr) const;
  FullWifiMode GetDataTxMode (Ptr<const Packet> packet, const FullWifiMacHeader *hdr) const;
  FullWifiMode GetCtsTxModeForRts (Mac48Address to, FullWifiMode rtsTxMode) const;
  FullWifiMode GetAckTxModeForData (Mac48Address to, FullWifiMode dataTxMode) const;

  Time GetCtsDuration (Mac48Address to, FullWifiMode rtsTxMode) const;
  Time GetAckDuration (Mac48Address to, FullWifiMode dataTxMode) const;
  Time GetBlockAckDuration (Mac48Address to, FullWifiMode blockAckReqTxMode, enum BlockAckType type) const;
  void NotifyNav (const FullWifiMacHeader &hdr, FullWifiMode txMode, FullWifiPreamble preamble);
  ...
  void SendCtsAfterRts (Mac48Address source, Time duration, FullWifiMode txMode, double rtsSnr);
  void SendAckAfterData (Mac48Address source, Time duration, FullWifiMode txMode, double rtsSnr);
  void SendDataAfterCts (Mac48Address source, Time duration, FullWifiMode txMode);
   ...
   */
  bool StoreMpduIfNeeded (Ptr<Packet> packet, FullWifiMacHeader hdr);
  /*
   * Invoked after that a block ack request has been received. Looks for corresponding
   * block ack agreement and creates block ack bitmap on a received packets basis.
   */
  void SendBlockAckAfterBlockAckRequest (const FullCtrlBAckRequestHeader reqHdr, Mac48Address originator,
                                         Time duration, FullWifiMode blockAckReqTxMode);
  /*
   * This method creates block ack frame with header equals to <i>blockAck</i> and start its transmission.
   */
  void SendBlockAckResponse (const FullCtrlBAckResponseHeader* blockAck, Mac48Address originator, bool immediate,
                             Time duration, FullWifiMode blockAckReqTxMode);
  /*
   ...
   */
  void ResetBlockAckInactivityTimerIfNeeded (FullBlockAckAgreement &agreement);

  void SetupPhyMacLowListener (Ptr<FullWifiPhy> phy);


  Ptr<FullWifiPhy> m_phy;
  Ptr<FullWifiRemoteStationManager> m_stationManager;
  MacLowRxCallback m_rxCallback;
  typedef std::vector<FullMacLowDcfListener *>::const_iterator DcfListenersCI;
  typedef std::vector<FullMacLowDcfListener *> DcfListeners;
  ...
  FullMacLowTransmissionParameters m_txParams;
  FullMacLowTransmissionListener *m_listener;
  Mac48Address m_self;
   ...
   */
  typedef std::pair<Ptr<Packet>, FullWifiMacHeader> BufferedPacket;
  typedef std::list<BufferedPacket>::iterator BufferedPacketI;

  typedef std::pair<Mac48Address, uint8_t> AgreementKey;
  typedef std::pair<FullBlockAckAgreement, std::list<BufferedPacket> > AgreementValue;

  typedef std::map<AgreementKey, AgreementValue> Agreements;
  typedef std::map<AgreementKey, AgreementValue>::iterator AgreementsI;

  typedef std::map<AgreementKey, FullBlockAckCache> BlockAckCaches;
  typedef std::map<AgreementKey, FullBlockAckCache>::iterator BlockAckCachesI;

  Agreements m_bAckAgreements;
  BlockAckCaches m_bAckCaches;

  typedef std::map<AcIndex, FullMacLowBlockAckEventListener*> QueueListeners;
  QueueListeners m_edcaListeners;

  //the end of duplex transmission (rx and tx)
  Time m_duplexEnd;
};

} // namespace ns3

#endif /* FULL_MAC_LOW_H */
