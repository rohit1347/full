...
NS_LOG_COMPONENT_DEFINE ("FullMsduAggregator");

namespace ns3 {

NS_OBJECT_ENSURE_REGISTERED (FullMsduAggregator);

TypeId
FullMsduAggregator::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::FullMsduAggregator")
    .SetParent<Object> ()
  ;
  return tid;
}

FullMsduAggregator::DeaggregatedMsdus
FullMsduAggregator::Deaggregate (Ptr<Packet> aggregatedPacket)
{
  NS_LOG_FUNCTION_NOARGS ();
  DeaggregatedMsdus set;

  FullAmsduSubframeHeader hdr;
  ...
  while (deserialized < maxSize)
    {
      ...
      std::pair<Ptr<Packet>, FullAmsduSubframeHeader> packetHdr (extractedMsdu, hdr);
