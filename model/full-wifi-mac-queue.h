 ...
 */
#ifndef FULL_WIFI_MAC_QUEUE_H
#define FULL_WIFI_MAC_QUEUE_H
 ...
 */
class FullWifiMacQueue : public Object
{
public:
  ...
  void Enqueue (Ptr<const Packet> packet, const FullWifiMacHeader &hdr);
  void PushFront (Ptr<const Packet> packet, const FullWifiMacHeader &hdr);
  Ptr<const Packet> Dequeue (FullWifiMacHeader *hdr);
  Ptr<const Packet> Peek (FullWifiMacHeader *hdr);
  /**
   ...
   * aggregation (A-MSDU).
   */
  Ptr<const Packet> DequeueByTidAndAddress (FullWifiMacHeader *hdr,
                                            uint8_t tid,
                                            FullWifiMacHeader::AddressType type,
                                            Mac48Address addr);
  /**
   * Searchs and returns, if is present in this queue, first packet having
   * address indicated by <i>type</i> equals to <i>addr</i>, and tid
   ...
   * aggregation (A-MSDU).
   */
  Ptr<const Packet> PeekByTidAndAddress (FullWifiMacHeader *hdr,
                                         uint8_t tid,
                                         FullWifiMacHeader::AddressType type,
                                         Mac48Address addr);
  /**
   ...
   */
  Ptr<const Packet> DequeueFirstAvailable (FullWifiMacHeader *hdr,
                                           Time &tStamp,
   ...
   */
  Ptr<const Packet> PeekFirstAvailable (FullWifiMacHeader *hdr,
                                        Time &tStamp,
                                        const FullQosBlockedDestinations *blockedPackets);

  /**
   * Check to see if there is a packet for the given destination
   * Return the first available packet and remove it
   */
  Ptr<const Packet> DequeueFirstAvailable (FullWifiMacHeader *hdr, Mac48Address src);
  Ptr<const Packet> PeekFirstAvailable (FullWifiMacHeader *hdr, Mac48Address src);
...
private:
  ...
  typedef std::list<struct FullItem> PacketQueue;
  typedef std::list<struct FullItem>::reverse_iterator PacketQueueRI;
  typedef std::list<struct FullItem>::iterator PacketQueueI;

  void Cleanup (void);
  Mac48Address GetAddressForPacket (enum FullWifiMacHeader::AddressType type, PacketQueueI);

  struct FullItem
  {
    FullItem (Ptr<const Packet> packet,
  ...
  PacketQueue m_queue;
  FullWifiMacParameters *m_parameters;
  uint32_t m_size;
  uint32_t m_maxSize;
  Time m_maxDelay;
};
