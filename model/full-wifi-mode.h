 ...
 */
#ifndef FULL_WIFI_MODE_H
#define FULL_WIFI_MODE_H
...
#include "ns3/full-wifi-phy-standard.h"
...
public:
   ...
   */
  enum FullWifiCodeRate GetCodeRate (void) const;
  /**
   ...
   */
  enum FullWifiModulationClass GetModulationClass () const;
   ...
   */
  FullWifiMode ();
  FullWifiMode (std::string name);
private:
  friend class FullWifiModeFactory;
  FullWifiMode (uint32_t uid);
  uint32_t m_uid;
};
...
ATTRIBUTE_HELPER_HEADER (FullWifiMode);
 ...
 */
typedef std::vector<FullWifiMode> FullWifiModeList;
typedef FullWifiModeList::const_iterator FullWifiModeListIterator;
...
public:
   ...
   * Create a WifiMode.
   */
  static FullWifiMode CreateWifiMode (std::string uniqueName,
...
private:
  friend class FullWifiMode;
  friend std::istream & operator >> (std::istream &is, FullWifiMode &mode);
  static FullWifiModeFactory* GetFactory ();
  FullWifiModeFactory ();
  ...
  FullWifiMode Search (std::string name);
  uint32_t AllocateUid (std::string uniqueName);
  FullWifiModeItem* Get (uint32_t uid);

  typedef std::vector<struct FullWifiModeItem> FullWifiModeItemList;
  FullWifiModeItemList m_itemList;
};

} // namespace ns3

#endif /* FULL_WIFI_MODE_H */
