 ...
 */
#ifndef FULL_BLOCK_ACK_MANAGER_H
#define FULL_BLOCK_ACK_MANAGER_H
...
#include "full-originator-block-ack-agreement.h"
...
class FullMgtAddBaRequestHeader;
...
{
private:
  FullBlockAckManager (const FullBlockAckManager&);
  FullBlockAckManager& operator= (const FullBlockAckManager&);
...
public:
   ...
   */
  void CreateAgreement (const FullMgtAddBaRequestHeader *reqHdr, Mac48Address recipient);
  /**
   * \param recipient Address of peer station involved in block ack mechanism.
   ...
   */
  void UpdateAgreement (const FullMgtAddBaResponseHeader *respHdr, Mac48Address recipient);
  /**
   ...
   */
  void StorePacket (Ptr<const Packet> packet, const FullWifiMacHeader &hdr, Time tStamp);
  /**
   ...
   */
  Ptr<const Packet> GetNextPacket (FullWifiMacHeader &hdr);
   ...
   */
  void NotifyGotBlockAck (const FullCtrlBAckResponseHeader *blockAck, Mac48Address recipient);
  /**
   * \param recipient Address of peer station involved in block ack mechanism.
   * \param tid Traffic ID.
   *
   ...
   */
  void SetQueue (Ptr<FullWifiMacQueue> queue);
...
private:
  ...
  typedef std::map<std::pair<Mac48Address, uint8_t>,
                   std::pair<FullOriginatorBlockAckAgreement, PacketQueue> > Agreements;
  typedef std::map<std::pair<Mac48Address, uint8_t>,
                   std::pair<FullOriginatorBlockAckAgreement, PacketQueue> >::iterator AgreementsI;
  typedef std::map<std::pair<Mac48Address, uint8_t>,
                   std::pair<FullOriginatorBlockAckAgreement, PacketQueue> >::const_iterator AgreementsCI;
...
#endif /* FULL_BLOCK_ACK_MANAGER_H */
