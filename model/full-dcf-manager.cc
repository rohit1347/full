...
NS_LOG_COMPONENT_DEFINE ("FullDcfManager");
...
void
FullDcfState::SetAifsn (uint32_t aifsn)
{
  m_aifsn = aifsn;
}
void
FullDcfState::SetCwMin (uint32_t minCw)
{
  m_cwMin = minCw;
  ResetCw ();
}
void
FullDcfState::SetCwMax (uint32_t maxCw)
{
  m_cwMax = maxCw;
  ResetCw ();
}
uint32_t
FullDcfState::GetAifsn (void) const
{
  return m_aifsn;
}
uint32_t
FullDcfState::GetCwMin (void) const
{
  return m_cwMin;
}
uint32_t
FullDcfState::GetCwMax (void) const
{
  return m_cwMax;
}

void
FullDcfState::ResetCw (void)
{
  m_cw = m_cwMin;
}
void
FullDcfState::UpdateFailedCw (void)
{
  // see 802.11-2007, section 9.9.1.5
  m_cw = std::min ( 2 * (m_cw + 1) - 1, m_cwMax);
}
void
FullDcfState::UpdateBackoffSlotsNow (uint32_t nSlots, Time backoffUpdateBound)
{
...
void
FullDcfState::ResetBackoffSlots (void)
{
  MY_DEBUG ("reset backoffslots to 0");
  m_backoffSlots = 0;
}

void
FullDcfState::StartBackoffNow (uint32_t nSlots)
{
...
uint32_t
FullDcfState::GetCw (void) const
{
  return m_cw;
}
uint32_t
FullDcfState::GetBackoffSlots (void) const
{
  return m_backoffSlots;
}
Time
FullDcfState::GetBackoffStart (void) const
{
  return m_backoffStart;
}
bool
FullDcfState::IsAccessRequested (void) const
{
  return m_accessRequested;
}
void
FullDcfState::NotifyAccessRequested (void)
{
  m_accessRequested = true;
}
void
FullDcfState::NotifyAccessGranted (void)
{
...
void
FullDcfState::NotifyCollision (void)
{
  DoNotifyCollision ();
}
void
FullDcfState::NotifyInternalCollision (void)
{
  DoNotifyInternalCollision ();
}
void
FullDcfState::NotifyChannelSwitching (void)
{
...
class FullLowDcfListener : public ns3::FullMacLowDcfListener
{
public:
  FullLowDcfListener (ns3::FullDcfManager *dcf)
    : m_dcf (dcf)
  {
  }
  virtual ~FullLowDcfListener ()
  {
  }
...
class FullPhyListener : public ns3::FullWifiPhyListener
{
public:
  FullPhyListener (ns3::FullDcfManager *dcf)
    : m_dcf (dcf)
  {
  }
  virtual ~FullPhyListener ()
  {
  }
  virtual void NotifyRxStart (Time duration, Ptr<const Packet> packet, FullWifiMode txMode, FullWifiPreamble preamble)
  {
    m_dcf->NotifyRxStartNow (duration, packet, txMode, preamble);
  }
...
void
FullDcfManager::SetupPhyListener (Ptr<FullWifiPhy> phy)
{
  m_phyListener = new FullPhyListener (this);
  phy->RegisterListener (m_phyListener);
}
void
FullDcfManager::SetupLowListener (Ptr<FullMacLow> low)
{
  m_lowListener = new FullLowDcfListener (this);
  low->RegisterDcfListener (m_lowListener);
}

void
FullDcfManager::SetSlot (Time slotTime)
{
  m_slotTimeUs = slotTime.GetMicroSeconds ();
}
void
FullDcfManager::SetSifs (Time sifs)
{
  m_sifs = sifs;
}
void
FullDcfManager::SetEifsNoDifs (Time eifsNoDifs)
{
  m_eifsNoDifs = eifsNoDifs;
}
Time
FullDcfManager::GetEifsNoDifs () const
{
  return m_eifsNoDifs;
}

void
FullDcfManager::Add (FullDcfState *dcf)
{
  m_states.push_back (dcf);
}

Time
FullDcfManager::MostRecent (Time a, Time b) const
{
  return Max (a, b);
}
Time
FullDcfManager::MostRecent (Time a, Time b, Time c) const
{
  Time retval;
  retval = Max (a, b);
  retval = Max (retval, c);
...
Time
FullDcfManager::MostRecent (Time a, Time b, Time c, Time d) const
{
  Time e = Max (a, b);
  Time f = Max (c, d);
  Time retval = Max (e, f);
...
Time
FullDcfManager::MostRecent (Time a, Time b, Time c, Time d, Time e, Time f) const
{
  Time g = Max (a, b);
  Time h = Max (c, d);
  Time i = Max (e, f);
...
Time
FullDcfManager::MostRecent (Time a, Time b, Time c, Time d, Time e, Time f, Time g) const
{
  Time h = Max (a, b);
  Time i = Max (c, d);
  Time j = Max (e, f);
...
bool
FullDcfManager::IsBusy (void) const
{
  // PHY busy
  if (m_rxing)
    {
...
void
FullDcfManager::RequestAccess (FullDcfState *state)
{
  UpdateBackoff ();
...
void
FullDcfManager::DoGrantAccess (void)
{
  uint32_t k = 0;
  // why are there multiple different states when only one mac, one dcp-txop is associated
  // ANSWER: because AP has another state for beacon while other nodes only have one state
  for (States::const_iterator i = m_states.begin (); i != m_states.end (); k++)
    {
      FullDcfState *state = *i;
      if (state->IsAccessRequested ()
          && GetBackoffEndFor (state) <= Simulator::Now () )
        {
          ...
          k++;
          std::vector<FullDcfState *> internalCollisionStates;
          for (States::const_iterator j = i; j != m_states.end (); j++, k++)
            {
              FullDcfState *otherState = *j;
          ...
          for (std::vector<FullDcfState *>::const_iterator k = internalCollisionStates.begin ();
...
void
FullDcfManager::AccessTimeout (void)
{
  UpdateBackoff ();
  DoGrantAccess ();
  DoRestartAccessTimeoutIfNeeded ();
...
Time
FullDcfManager::GetAccessGrantStart (void) const
{
  Time rxAccessStart;
  if (!m_rxing)
    {
...
Time
FullDcfManager::GetBackoffStartFor (FullDcfState *state)
{
...
Time
FullDcfManager::GetBackoffEndFor (FullDcfState *state)
{
  return GetBackoffStartFor (state) + MicroSeconds (state->GetBackoffSlots () * m_slotTimeUs);
}

void
FullDcfManager::UpdateBackoff (void)
{
  uint32_t k = 0;
...
void
FullDcfManager::DoRestartAccessTimeoutIfNeeded (void)
{
  /**
      ...
      if (m_accessTimeout.IsExpired ())
        {
          m_accessTimeout = Simulator::Schedule (expectedBackoffDelay,
                                                 &FullDcfManager::AccessTimeout, this);
        }
    }
}

void
FullDcfManager::NotifyRxStartNow (Time duration, Ptr<const Packet> packet, FullWifiMode txMode, FullWifiPreamble preamble)
{
  //add the reaction to the incoming packet
  //notify the mac layer abou the new arriving packet
  // this should be done before setting m_rxing to true,
  // otherwise the function NotifyTxStartNow has to be taken
  // special care for sending return transmission.
  for (States::const_iterator i = m_states.begin (); i != m_states.end (); i++)
    {
      FullDcfState *state = *i;
      state->DoNotifyRxStartNow(duration, packet, txMode, preamble);
    }
...
void
FullDcfManager::NotifyRxEndOkNow (void)
{
  MY_DEBUG ("rx end ok");
...
void
FullDcfManager::NotifyRxEndErrorNow (void)
{
...
void
FullDcfManager::NotifyTxStartNow (Time duration)
{
  //FIXME: should be able to receive and transmit at the same time
  // for sending return transmission when m_rxing is true.
  // However, since NotifyRxStartNow process state->DoNotifyRxStartNow
  // before touching the value m_rxing, it is fine to leave this part
  // upchanged.
  // Since we are able to receive and transmit at the same time, it is
  // ok if m_rxing is true
//  if (m_rxing)
//    {
...
void
FullDcfManager::NotifyMaybeCcaBusyStartNow (Time duration)
{
...
void
FullDcfManager::NotifySwitchingStartNow (Time duration)
{
...
void
FullDcfManager::NotifyNavResetNow (Time duration)
{
...
void
FullDcfManager::NotifyNavStartNow (Time duration)
{
...
void
FullDcfManager::NotifyAckTimeoutStartNow (Time duration)
{
  NS_ASSERT (m_lastAckTimeoutEnd < Simulator::Now ());
  m_lastAckTimeoutEnd = Simulator::Now () + duration;
}
void
FullDcfManager::NotifyAckTimeoutResetNow ()
{
  m_lastAckTimeoutEnd = Simulator::Now ();
  DoRestartAccessTimeoutIfNeeded ();
}
void
FullDcfManager::NotifyCtsTimeoutStartNow (Time duration)
{
  m_lastCtsTimeoutEnd = Simulator::Now () + duration;
}
void
FullDcfManager::NotifyCtsTimeoutResetNow ()
{
