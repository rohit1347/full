...
#ifndef FULL_DCF_MANAGER_H
#define FULL_DCF_MANAGER_H
...
public:
  ...
  /**
   * reset the m_backoffSlots to 0
   */
  void ResetBackoffSlots (void);

private:
  friend class FullDcfManager;
  ...
  /**
   * Called by DcfManger to notify a DcfState subclss
   * that a new packet is received. This function is for full duplex use.
   * The received packet could be used by the mac layer to take some reaction.
   */
  virtual void DoNotifyRxStartNow (Time duration, Ptr<const Packet> packet, FullWifiMode txMode, FullWifiPreamble preamble) = 0;
...
public:
  ...
  void SetupPhyListener (Ptr<FullWifiPhy> phy);
  void SetupLowListener (Ptr<FullMacLow> low);
   ...
   */
  void RequestAccess (FullDcfState *state);
...
private:
  ...
  Time GetBackoffStartFor (FullDcfState *state);
  Time GetBackoffEndFor (FullDcfState *state);
  void DoRestartAccessTimeoutIfNeeded (void);
  void AccessTimeout (void);
  void DoGrantAccess (void);
  bool IsBusy (void) const;

  typedef std::vector<FullDcfState *> States;
  ...
  Time m_sifs;
  FullPhyListener* m_phyListener;
  FullLowDcfListener* m_lowListener;
};

} // namespace ns3

#endif /* FULL_DCF_MANAGER_H */
