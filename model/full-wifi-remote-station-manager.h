 ...
 */
#ifndef FULL_WIFI_REMOTE_STATION_MANAGER_H
#define FULL_WIFI_REMOTE_STATION_MANAGER_H
...
struct FullWifiRemoteStation;
struct FullWifiRemoteStationState;
class FullWifiPhy;
 ...
 */
class FullWifiRemoteStationInfo
{
public:
 ...
 */
class FullWifiRemoteStationManager : public Object
{
public:
  static TypeId GetTypeId (void);

  FullWifiRemoteStationManager ();
  virtual ~FullWifiRemoteStationManager ();
  ...
  void AddBasicMode (FullWifiMode mode);

  FullWifiMode GetDefaultMode (void) const;
  uint32_t GetNBasicModes (void) const;
  FullWifiMode GetBasicMode (uint32_t i) const;

  FullWifiMode GetNonUnicastMode (void) const;
   ...
   * the BSSBasicRateSet.
   */
  void AddSupportedMode (Mac48Address address, FullWifiMode mode);
   ...
   */
  void PrepareForQueue (Mac48Address address, const FullWifiMacHeader *header,
   ...
   */
  FullWifiMode GetDataMode (Mac48Address address, const FullWifiMacHeader *header,
                        Ptr<const Packet> packet, uint32_t fullPacketSize);
  /**
   * \param address remote address
   * \param header MAC header
   ...
   */
  FullWifiMode GetRtsMode (Mac48Address address, const FullWifiMacHeader *header,
                       Ptr<const Packet> packet);
  /**
   * Should be invoked whenever the RtsTimeout associated to a transmission
   * attempt expires.
   */
  void ReportRtsFailed (Mac48Address address, const FullWifiMacHeader *header);
  /**
   * Should be invoked whenever the AckTimeout associated to a transmission
   * attempt expires.
   */
  void ReportDataFailed (Mac48Address address, const FullWifiMacHeader *header);
  /**
   * Should be invoked whenever we receive the Cts associated to an RTS
   * we just sent.
   */
  void ReportRtsOk (Mac48Address address, const FullWifiMacHeader *header,
   ...
   * we just sent.
   */
  void ReportDataOk (Mac48Address address, const FullWifiMacHeader *header,
   ...
   */
  void ReportFinalRtsFailed (Mac48Address address, const FullWifiMacHeader *header);
  /**
   * Should be invoked after calling ReportDataFailed if
   * NeedDataRetransmission returns false
   */
  void ReportFinalDataFailed (Mac48Address address, const FullWifiMacHeader *header);
   ...
   */
  void ReportRxOk (Mac48Address address, const FullWifiMacHeader *header,
   ...
   */
  bool NeedRts (Mac48Address address, const FullWifiMacHeader *header,
                Ptr<const Packet> packet);
  /**
   * \param address remote address
   * \param header MAC header
   ...
   */
  bool NeedRtsRetransmission (Mac48Address address, const FullWifiMacHeader *header,
                              Ptr<const Packet> packet);
  /**
   * \param address remote address
   * \param header MAC header
   ...
   */
  bool NeedDataRetransmission (Mac48Address address, const FullWifiMacHeader *header,
                               Ptr<const Packet> packet);
   ...
   */
  bool NeedFragmentation (Mac48Address address, const FullWifiMacHeader *header,
                          Ptr<const Packet> packet);
  /**
   * \param address remote address
   * \param header MAC header
   ...
   */
  uint32_t GetFragmentSize (Mac48Address address, const FullWifiMacHeader *header,
   ...
   */
  uint32_t GetFragmentOffset (Mac48Address address, const FullWifiMacHeader *header,
                              Ptr<const Packet> packet, uint32_t fragmentNumber);
  /**
   * \param address remote address
   * \param header MAC header
   ...
   */
  bool IsLastFragment (Mac48Address address, const FullWifiMacHeader *header,
                       Ptr<const Packet> packet, uint32_t fragmentNumber);
   ...
   *          handshake.
   */
  FullWifiMode GetCtsMode (Mac48Address address, FullWifiMode rtsMode);
  /**
   * \param address
   ...
   *          handshake.
   */
  FullWifiMode GetAckMode (Mac48Address address, FullWifiMode dataMode);

  FullWifiRemoteStationInfo GetInfo (Mac48Address address);
protected:
  virtual void DoDispose (void);
  // for convenience
  FullWifiMode GetSupported (const FullWifiRemoteStation *station, uint32_t i) const;
  uint32_t GetNSupported (const FullWifiRemoteStation *station) const;
private:
  /**
   ...
   *       and has failed.
   */
  virtual bool DoNeedRtsRetransmission (FullWifiRemoteStation *station,
                                        Ptr<const Packet> packet, bool normally);
  /**
   * \param station the station with which we need to communicate
   * \param packet the packet to send
   ...
   *       and has failed.
   */
  virtual bool DoNeedDataRetransmission (FullWifiRemoteStation *station,
                                         Ptr<const Packet> packet, bool normally);
   ...
   */
  virtual bool DoNeedFragmentation (FullWifiRemoteStation *station,
                                    Ptr<const Packet> packet, bool normally);
  /**
   ...
   */
  virtual FullWifiRemoteStation* DoCreateStation (void) const = 0;
  /**
   * \param station the station with which we need to communicate
   ...
   */
  virtual FullWifiMode DoGetDataMode (FullWifiRemoteStation *station,
                                  uint32_t size) = 0;
  /**
   * \param station the station with which we need to communicate
   ...
   */
  virtual FullWifiMode DoGetRtsMode (FullWifiRemoteStation *station) = 0;
  virtual void DoReportRtsFailed (FullWifiRemoteStation *station) = 0;
  virtual void DoReportDataFailed (FullWifiRemoteStation *station) = 0;
  virtual void DoReportRtsOk (FullWifiRemoteStation *station,
                              double ctsSnr, FullWifiMode ctsMode, double rtsSnr) = 0;
  virtual void DoReportDataOk (FullWifiRemoteStation *station,
                               double ackSnr, FullWifiMode ackMode, double dataSnr) = 0;
  virtual void DoReportFinalRtsFailed (FullWifiRemoteStation *station) = 0;
  virtual void DoReportFinalDataFailed (FullWifiRemoteStation *station) = 0;
  virtual void DoReportRxOk (FullWifiRemoteStation *station,
                             double rxSnr, FullWifiMode txMode) = 0;

  FullWifiRemoteStationState* LookupState (Mac48Address address) const;
  FullWifiRemoteStation* Lookup (Mac48Address address, uint8_t tid) const;
  /// Find a remote station by its remote address and TID taken from MAC header
  FullWifiRemoteStation* Lookup (Mac48Address address, const FullWifiMacHeader *header) const;
  FullWifiMode GetControlAnswerMode (Mac48Address address, FullWifiMode reqMode);

  void DoSetFragmentationThreshold (uint32_t threshold);
  uint32_t DoGetFragmentationThreshold (void) const;
  uint32_t GetNFragments (const FullWifiMacHeader *header, Ptr<const Packet> packet);

  typedef std::vector <FullWifiRemoteStation *> Stations;
  typedef std::vector <FullWifiRemoteStationState *> StationStates;
   ...
   * "DeviceRateSet").
   */
  Ptr<FullWifiPhy> m_wifiPhy;
  FullWifiMode m_defaultTxMode;

  /**
   * This member is the list of FullWifiMode objects that comprise the
   ...
   */
  FullWifiModeList m_bssBasicRateSet;
...
struct FullWifiRemoteStationState
{
   ...
   */
  FullWifiModeList m_operationalRateSet;

  Mac48Address m_address;
  FullWifiRemoteStationInfo m_info;
};
...
struct FullWifiRemoteStation
{
  FullWifiRemoteStationState *m_state;
  uint32_t m_ssrc;
  uint32_t m_slrc;
  uint8_t m_tid;
};
...
#endif /* FULL_WIFI_REMOTE_STATION_MANAGER_H */
