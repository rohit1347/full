...
FullMacTxMiddle::FullMacTxMiddle ()
  : m_sequence (0)
{
}
...
uint16_t
FullMacTxMiddle::GetNextSequenceNumberfor (const FullWifiMacHeader *hdr)
{
  uint16_t retval;
  if (hdr->IsQosData ()
...
uint16_t
FullMacTxMiddle::GetNextSeqNumberByTidAndAddress (uint8_t tid, Mac48Address addr) const
{
  NS_ASSERT (tid < 16);
  uint16_t seq = 0;
