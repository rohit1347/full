...
private:
  void Send (void);
  void Receive (Ptr<Packet> p, double snr, FullWifiMode mode, enum FullWifiPreamble preamble);
  Ptr<FullWifiPhy> m_tx;
...
PsrExperiment::Send (void)
{
  ...
  FullWifiMode mode = FullWifiMode (m_input.txMode);
  m_tx->SendPacket (p, mode, FULL_WIFI_PREAMBLE_SHORT, m_input.txPowerLevel);
}

void
PsrExperiment::Receive (Ptr<Packet> p, double snr, FullWifiMode mode, enum FullWifiPreamble preamble)
{
  m_output.received++;
}
...
PsrExperiment::Run (struct PsrExperiment::Input input)
{
  ...
  Ptr<FullYansWifiPhy> tx = CreateObject<FullYansWifiPhy> ();
  Ptr<FullYansWifiPhy> rx = CreateObject<FullYansWifiPhy> ();
...
private:
  ...
  void Receive (Ptr<Packet> p, double snr, FullWifiMode mode, enum FullWifiPreamble preamble);
  Ptr<FullWifiPhy> m_txA;
  Ptr<FullWifiPhy> m_txB;
  uint32_t m_flowIdA;
  uint32_t m_flowIdB;
  struct Input m_input;
  struct Output m_output;
...
CollisionExperiment::SendA (void) const
{
  ...
  m_txA->SendPacket (p, FullWifiMode (m_input.txModeA),
                     FULL_WIFI_PREAMBLE_SHORT, m_input.txPowerLevelA);
}
...
CollisionExperiment::SendB (void) const
{
  ...
  m_txB->SendPacket (p, FullWifiMode (m_input.txModeB),
                     FULL_WIFI_PREAMBLE_SHORT, m_input.txPowerLevelB);
}

void
CollisionExperiment::Receive (Ptr<Packet> p, double snr, FullWifiMode mode, enum FullWifiPreamble preamble)
{
  FlowIdTag tag;
...
CollisionExperiment::Run (struct CollisionExperiment::Input input)
{
  ...
  Ptr<FullYansWifiPhy> txA = CreateObject<FullYansWifiPhy> ();
  Ptr<FullYansWifiPhy> txB = CreateObject<FullYansWifiPhy> ();
  Ptr<FullYansWifiPhy> rx = CreateObject<FullYansWifiPhy> ();
