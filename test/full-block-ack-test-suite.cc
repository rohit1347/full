...
#include "ns3/test.h"
#include "ns3/log.h"
#include "ns3/full-qos-utils.h"
#include "ns3/full-ctrl-headers.h"
#include <list>

using namespace ns3;

NS_LOG_COMPONENT_DEFINE ("FullBlockAckTest");
/**
 ...
 *
 */
class FullPacketBufferingCaseA : public TestCase
{
public:
  FullPacketBufferingCaseA ();
  virtual ~FullPacketBufferingCaseA ();
private:
  virtual void DoRun (void);
  std::list<uint16_t> m_expectedBuffer;
};

FullPacketBufferingCaseA::FullPacketBufferingCaseA ()
...
void
FullPacketBufferingCaseA::DoRun (void)
{
 ...
 *
 */
class FullPacketBufferingCaseB : public TestCase
{
public:
  FullPacketBufferingCaseB ();
  virtual ~FullPacketBufferingCaseB ();
private:
  virtual void DoRun (void);
  std::list<uint16_t> m_expectedBuffer;
};

FullPacketBufferingCaseB::FullPacketBufferingCaseB ()
  : TestCase ("Check correct order of buffering when startSequence > endSeq")
{
...
void
FullPacketBufferingCaseB::DoRun (void)
{
  std::list<uint16_t> m_buffer;
  std::list<uint16_t>::iterator i,j;
...
class FullCtrlBAckResponseHeaderTest : public TestCase
{
public:
  FullCtrlBAckResponseHeaderTest ();
private:
  virtual void DoRun ();
  FullCtrlBAckResponseHeader m_blockAckHdr;
};

FullCtrlBAckResponseHeaderTest::FullCtrlBAckResponseHeaderTest ()
  : TestCase ("Check the correctness of block ack compressed bitmap")
{
}

void
FullCtrlBAckResponseHeaderTest::DoRun (void)
{
...
class FullBlockAckTestSuite : public TestSuite
{
public:
  FullBlockAckTestSuite ();
};

FullBlockAckTestSuite::FullBlockAckTestSuite ()
  : TestSuite ("wifi-block-ack", UNIT)
{
  AddTestCase (new FullPacketBufferingCaseA);
  AddTestCase (new FullPacketBufferingCaseB);
  AddTestCase (new FullCtrlBAckResponseHeaderTest);
}

static FullBlockAckTestSuite g_blockAckTestSuite;
