...
#include "full-yans-wifi-helper.h"
...
NS_LOG_COMPONENT_DEFINE ("FullYansWifiHelper");
...
FullYansWifiChannelHelper::FullYansWifiChannelHelper ()
{
}

FullYansWifiChannelHelper
FullYansWifiChannelHelper::Default (void)
{
  FullYansWifiChannelHelper helper;
...
void
FullYansWifiChannelHelper::AddPropagationLoss (std::string type,
...
void
FullYansWifiChannelHelper::SetPropagationDelay (std::string type,
                                            std::string n0, const AttributeValue &v0,
                                            std::string n1, const AttributeValue &v1,
                                            std::string n2, const AttributeValue &v2,
                                            std::string n3, const AttributeValue &v3,
...
Ptr<FullYansWifiChannel>
FullYansWifiChannelHelper::Create (void) const
{
...
int64_t 
FullYansWifiChannelHelper::AssignStreams (Ptr<FullYansWifiChannel> c, int64_t stream)
{
  return c->AssignStreams (stream);
}

FullYansWifiPhyHelper::FullYansWifiPhyHelper ()
  : m_channel (0),
    m_pcapDlt (PcapHelper::DLT_IEEE802_11)
{
  m_phy.SetTypeId ("ns3::FullYansWifiPhy");
}

FullYansWifiPhyHelper
FullYansWifiPhyHelper::Default (void)
{
  FullYansWifiPhyHelper helper;
  helper.SetErrorRateModel ("ns3::FullNistErrorRateModel");
  return helper;
}

void
FullYansWifiPhyHelper::SetChannel (Ptr<FullYansWifiChannel> channel)
{
  m_channel = channel;
}
void
FullYansWifiPhyHelper::SetChannel (std::string channelName)
{
  Ptr<FullYansWifiChannel> channel = Names::Find<FullYansWifiChannel> (channelName);
  m_channel = channel;
}
void
FullYansWifiPhyHelper::Set (std::string name, const AttributeValue &v)
{
  m_phy.Set (name, v);
}

void
FullYansWifiPhyHelper::SetErrorRateModel (std::string name,
                                      std::string n0, const AttributeValue &v0,
                                      std::string n1, const AttributeValue &v1,
                                      std::string n2, const AttributeValue &v2,
                                      std::string n3, const AttributeValue &v3,
...
Ptr<FullWifiPhy>
FullYansWifiPhyHelper::Create (Ptr<Node> node, Ptr<FullWifiNetDevice> device) const
{
  Ptr<FullYansWifiPhy> phy = m_phy.Create<FullYansWifiPhy> ();
  Ptr<FullErrorRateModel> error = m_errorRateModel.Create<FullErrorRateModel> ();
...
void
FullYansWifiPhyHelper::SetPcapDataLinkType (enum SupportedPcapDataLinkTypes dlt)
{
  switch (dlt)
    {
    case DLT_IEEE802_11:
    ...
    case DLT_IEEE802_11_RADIO:
      ...
      return;
    default:
      NS_ABORT_MSG ("FullYansWifiPhyHelper::SetPcapFormat(): Unexpected format");
    }
}

void
FullYansWifiPhyHelper::EnablePcapInternal (std::string prefix, Ptr<NetDevice> nd, bool promiscuous, bool explicitFilename)
{
  //
  ...
  //
  Ptr<FullWifiNetDevice> device = nd->GetObject<FullWifiNetDevice> ();
  if (device == 0)
    {
      NS_LOG_INFO ("FullYansWifiHelper::EnablePcapInternal(): Device " << &device << " not of type ns3::WifiNetDevice");
      return;
    }

  Ptr<FullWifiPhy> phy = device->GetPhy ();
  NS_ABORT_MSG_IF (phy == 0, "FullYansWifiPhyHelper::EnablePcapInternal(): Phy layer in WifiNetDevice must be set");
...
void
FullYansWifiPhyHelper::EnableAsciiInternal (
  Ptr<OutputStreamWrapper> stream,
  std::string prefix,
  Ptr<NetDevice> nd,
  bool explicitFilename)
{
  ...
  if (device == 0)
    {
      NS_LOG_INFO ("FullYansWifiHelper::EnableAsciiInternal(): Device " << device << " not of type ns3::WifiNetDevice");
      return;
    }
  ...
  if (stream == 0)
    {
      ...
      //
      oss.str ("");
      oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid << "/$ns3::FullWifiNetDevice/Phy/State/RxOk";
      Config::ConnectWithoutContext (oss.str (), MakeBoundCallback (&AsciiPhyReceiveSinkWithoutContext, theStream));

      oss.str ("");
      oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid << "/$ns3::FullWifiNetDevice/Phy/State/Tx";
